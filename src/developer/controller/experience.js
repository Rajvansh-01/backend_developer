...
class experienceController{
    static addExperience = async(req, res) => {
        try {
            const {companyName, designation, startDate, endDate, location, jobType, description, licenseName, issuingOrg,
                startDateLicense,
                endDateLicense,
                credentialID, 
                projectName,
            ...
            if(companyName && designation && startDate && endDate && location && jobType && description && 
                licenseName && issuingOrg && startDateLicense && endDateLicense && credentialID && projectName && projectURL && 
                startDateProject && endDateProject && projectDescription){
                try {
                    ...
                    experienceInfo.save().then(info => {
                        ...
                        personalInfo.findByIdAndUpdate(personID, {experienceID: experienceID}, {new: true}).then(personalInfo => {
                            if(personalInfo){
                                res.status(200).json({status:"success", message:"Experience details added successfully!", data: personalInfo});
                            }else{
